...
Statut (2026-06-26): **complète** sur `codex/generation-persistence-queue`. Les 5 livrables sont implémentés et validés (édition et variantes prouvées end-to-end sur l'infra réelle).
...
- variantes parallèles ✅ (`generate_variants` → N versions sœurs op "variant" + mode Variants dans le chat);
...
Statut (2026-06-26): **complète** sur `codex/generation-persistence-queue`. Exports déclenchables depuis la PreviewPanel d'un écran.
...
- export HTML autonome ✅ (download du document complet);
- export React/Tailwind ✅ (`designSpecToReact` → composant .tsx);
- zip téléchargeable ✅ (`buildViteProjectZip` via JSZip — projet Vite runnable);
- README d'intégration ✅ (inclus dans le bundle Vite).
